...
function formatItraIndexInclusion(status: string | undefined): string {
  return status === "available" ? "included" : "not included";
}

function formatWeatherForecastInclusion(status: string | undefined): string {
  return status === "available" ? "included" : "not included";
}
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/5 p-4">
      ...
      <div className="grid grid-cols-2 gap-2 text-sm text-blue-100/75">
        ...
        <p>
          <span className="text-blue-100/55">ITRA Index:</span>{" "}
          {formatItraIndexInclusion(estimation.derivedMetrics.externalSignals?.itra.status)}
        </p>
        <p>
          <span className="text-blue-100/55">Weather forecast:</span>{" "}
          {formatWeatherForecastInclusion(estimation.derivedMetrics.externalSignals?.weather.status)}
        </p>
      </div>
    </div>
  );
