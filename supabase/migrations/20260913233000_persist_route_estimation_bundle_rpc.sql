...
begin
  ...
  on conflict (user_id, route_hash, profile_signature) do update
  set
    estimated_time_minutes = excluded.estimated_time_minutes,
    difficulty = excluded.difficulty,
    average_slope_percent = excluded.average_slope_percent,
...
revoke all on function public.persist_route_estimation_bundle(
  uuid,
  text,
  text,
  integer,
...
grant execute on function public.persist_route_estimation_bundle(
  uuid,
  text,
  text,
  integer,
